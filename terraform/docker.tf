...
resource "docker_container" "airflow_webserver" {
    ...
    env = [
        ...
        "AIRFLOW_VAR_DATABRICKS_JOB_SILVER_FLIGHTS=${databricks_job.spark_jobs["silver_flights"].id}",
        "AIRFLOW_VAR_DATABRICKS_JOB_ANOMALY_DETECT=${databricks_job.spark_jobs["anomaly_detect"].id}",
        "AIRFLOW_VAR_DATABRICKS_JOB_GOLD_AGGREGATES=${databricks_job.spark_jobs["gold_aggregates"].id}",
